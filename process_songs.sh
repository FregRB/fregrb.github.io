...
for dir in "$parent_directory"/*/; do
    ...
    if [ -f "$dir/song.yml" ]; then
        ...
        guitar_path=$( /Users/matthewfregnan/Scripts/CHOpt.CLI.1.6.1.x64.Mac/1.8.1/fnf_chopt -f *.mid --lazy 1000000 --early-whammy 0 --no-image --engine rb  | \
        grep -v "Optimising" | \
        sed -e 's/ ([^(]*)//g' | \
        ...
        # Guitar Score
        guitar_score=$( /Users/matthewfregnan/Scripts/CHOpt.CLI.1.6.1.x64.Mac/1.8.1/CHOpt -f *.mid --early-whammy 0 --engine fnf -o "$guitar_output" | \
        awk '/^Total score:/ {print $NF; exit}' )

        # Assign the Bass path output of CHOpt command to the variable $bass_path
        bass_path=$( /Users/matthewfregnan/Scripts/CHOpt.CLI.1.6.1.x64.Mac/1.8.1/fnf_chopt -f *.mid -i bass --lazy 100000 --no-image --early-whammy 0 --engine rb -o "$bass_output" | \
        grep -v "Optimising" | \
        sed -e 's/ ([^(]*)//g' | \
        awk '/^Total score:/ {next} !/^Path:|^No SP score:/ {gsub(/: /, "/", $0); gsub(/: /, ",", $0); gsub("/ ", "/", $0); if (NR > 1 && items) printf ", "; printf "%s", $0; items=1} END {if (NR > 0) printf "\n"}' )
        ...
        # Bass Score
        bass_score=$( /Users/matthewfregnan/Scripts/CHOpt.CLI.1.6.1.x64.Mac/1.8.1/CHOpt -f *.mid -i bass --early-whammy 0 --engine fnf -o "$bass_output" | \
        awk '/^Total score:/ {print $NF; exit}' )
