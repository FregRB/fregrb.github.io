...
const usernameColors = [
    "#D2691E", "#4682B4", "#6B8E23", "#C71585", "#9370DB",
    "#DAA520", "#FF7F50", "#20B2AA", "#9ACD32", "#DB7093",
    "#7B68EE", "#5F9EA0", "#CD853F", "#C71585", "#32CD32"
];
...
if (showUsername) {
    ...
    if (data.message.displayName.toLowerCase() === data.message.username.toLowerCase()) {
        displayText = data.message.displayName;
    } else {
        displayText = `${data.message.displayName} (${data.message.username})`;
    }

    usernameDiv.innerText = displayText;

    // Pick a “fake-random” color based on the first character
    const firstChar = data.message.displayName.charCodeAt(0); // get ASCII code
    const colorIndex = firstChar % usernameColors.length; // map to 0-14
    usernameDiv.style.color = usernameColors[colorIndex];
}
...
const twitchColors = [
    "#D2691E", "#4682B4", "#6B8E23", "#C71585", "#9370DB",
    "#DAA520", "#FF7F50", "#20B2AA", "#9ACD32", "#DB7093",
    "#8d80daff", "#5F9EA0", "#CD853F", "#C71585", "#32CD32"
];

// Apply to username in TikTokChat
if (showUsername) {
    usernameDiv.innerText = data.nickname;

    // Get first character code
    const firstCharCode = data.nickname.charCodeAt(0);
    
    // Pick a color from the 15-color palette
    const colorIndex = firstCharCode % twitchColors.length;
    const color = twitchColors[colorIndex];
	...
	if (showBadges) {
		...
		if (data.isSubscriber) {
			...
			subscriberBanner.innerText = "SUB";
